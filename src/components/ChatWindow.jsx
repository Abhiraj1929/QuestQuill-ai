...
export default function ChatWindow() {
  ...
  const getReply = async () => {
    ...
    try {
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          message: trimmed,
          threadId: currThreadId || threadId || null,
        }),
      });
  ...
  // When reply arrives, push user+assistant messages in order
  useEffect(() => {
    if (!reply) return;
    const sent = lastSentPromptRef.current;
    if (!sent) return;
    ...
    lastSentPromptRef.current = ""; // reset
  }, [reply, setPrevChats]);
  ...
  return (
    <div className="bg-[#212121] h-screen w-full flex flex-col justify-between items-center text-center">
      ...
      <div className="w-full flex justify-between items-center">
        <span className="m-4 sm:ml-40">QuestQuill-Ai</span>
        <div className="m-4">
      ...
      <div className="w-full flex flex-col justify-center items-center">
        <form onSubmit={onSubmit} className="w-full relative max-w-[700px] flex justify-between items-center">
          <input
        ...
        <p className="text-sm py-2 px-0 text-[#b4b4b4]">
          QuestQuill-Ai can make mistakes. Check important info. See Cookie Preferences.
        </p>
      </div>
    </div>
  );
